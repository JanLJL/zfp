...
//
template<typename T>
size_t encode(uint dims[4], int4 stride, int bits_per_block, T *d_data, Word *d_stream, cudaStream_t custream)
{
...
void *setup_device_field_compress(const zfp_field *field, const int4 &stride, long long int &offset, cudaStream_t custream)
{
...
void *setup_device_field_decompress(const zfp_field *field, const int4 &stride, long long int &offset, cudaStream_t custream)
{
  bool field_device = cuZFP::is_gpu_ptr(field->data);
